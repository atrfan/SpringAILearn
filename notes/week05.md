...
- 实际投入：
- 今日目标：本周收口——笔记整理、思考题、部署反思、技术债务清单、第六周准备清单。
- 完成内容：
  - 回答本周思考题（记忆”记住对话” vs “不知道外部知识”的边界），见下文「本周思考题」节；
  - 部署视角反思（内存版重启失忆 / 多实例不共享），见「部署视角反思」节；
  - 汇总技术债务 7 项，见「技术债务清单」节——其中 #2「按条数裁剪对超长消息失效」由 Day34 实测 token 冲到 5166 提供真实证据；
  - 列出第六周（工具调用）准备事项 5 项，见「第六周准备清单」节。
- 产出路径：`notes/week05.md`（本记录 + 四个新增小节）。
- 测试或实验结果：本轮无代码、无 API，纯收口整理。
- 遇到的问题：无。
- 明日调整：进入第六周——工具调用；优先偿还技术债 #1（conversationId 归属校验），正好呼应”模型决策 vs 应用授权”主题。

## 本周思考题

> **会话记忆让模型”记住”了什么，又没能让它”知道”什么？**

会话记忆让模型”记住”的，是**我们这次对话里说过的话**——它把历史轮次（我问的、模型答的）作为上下文塞进下一次调用，让模型知道对话进行到哪了。所以它能跨轮引用（”我叫什么”→答出上文说过的名字）、能做指代消解（”它”指向上文提过的东西）。代价是：随着对话历史变长，context 变大，每轮所需的 token 也随之增加（Day34 实测 77→166→251）；而能记多少是有限的，靠对话轮数的窗口来控制，超过窗口的旧对话会被截断丢弃。

但会话记忆**没能让模型”知道”训练集之外的新事实**。它塞进去的只是”我们聊过的内容”，不是新的知识或参数——模型答题依旧只靠两样东西：**它训练时学到的知识 + 当前上下文窗口里的内容**。所以记忆能让它”记住我们聊过啥”，却不能让它”知道它本来就不知道的事”。

这条边界和第 4 周”结构化输出**保证形状、不保证内容真实**”是同一个方向：每种机制只负责它那一层，别指望它兼管别的。而”如何让模型知道训练集之外的外部知识”，正是**第 7 周 RAG** 要解决的问题。

## 部署视角反思

> **内存版 ChatMemory 在重启 / 多实例下会怎样？**

**服务重启：** 本周用的 `InMemoryChatMemoryRepository` 把所有会话历史存在 JVM 堆内存里。一旦服务重启（发版、崩溃、扩缩容），这块内存连同**全部 conversationId 的历史一起清零**——重启前聊到一半的会话，重启后模型完全”失忆”，用户再追问上文就答不出了。记忆的”生命周期”被绑在了进程的生命周期上。

**多实例部署：** 内存记忆只活在**单个进程**里，各实例互不共享。多实例 + 负载均衡时，同一个 conversationId 的不同轮次可能被路由到**不同实例**：第 1 轮落到实例 A（历史写进 A 的内存）、第 2 轮落到实例 B（B 的内存里根本没有这条会话）——于是模型时而记得、时而失忆，记忆串不起来。要稳，要么把同一会话粘在同一实例（session affinity，治标且脆弱），要么根本上换共享存储。

**结论：** 这两点正是”**为什么后续需要持久化 + 共享的 ChatMemory repository**”的直接理由——把记忆从”进程内存”搬到进程外的共享存储（如数据库 / Redis），重启不丢、多实例可共享。对应技术债 **#3（重启即失忆）** 和 **#4（多实例不共享）**。

## 技术债务清单

本周刻意只做内存版，以下为累积的技术债，按优先级/关联主题排列：

1. **conversationId 归属校验缺失**（决定 1）——生产接入登录鉴权后，须校验 conversationId 属于当前用户，否则方案 A 存在跨用户读历史的越权风险。当前单用户学习环境豁免。
2. **按条数裁剪对超长消息失效 / token 成本无上限**（Day34 实证）——条数窗口只界定轮数、不界定 token；单条超长消息可撑爆上下文而窗口毫无察觉，Day34 实测 token 冲到 **5166**。真实场景（长文档 / RAG 上下文）需改按 token 裁剪。
3. **内存版重启即失忆**——`InMemoryChatMemoryRepository` 在 JVM 堆内，服务重启历史全丢。需持久化 repository。
4. **多实例不共享记忆**——记忆在单进程，多实例部署时同一 conversationId 落到不同实例会读到不同 / 空历史。需外部共享存储。
5. **流式端点尚未接入记忆**——`/api/chat/stream` 仍无状态，记忆只在 `/api/conversation`。
6. **业务持久化与 ChatMemory 未打通**——本周只做”给模型看”的内存版，”给人看”的业务对话记录未做。
7. *（验证债）* **advisor 临界轮 ±1 时序未坐实**——决定 2 留下的”答话时先存 user 再取历史”，裁剪临界点可能可见 4 轮而非 5 轮，Day34 未专门构造样本验证。

## 第六周准备清单

承接本周债务 + 衔接第六周「工具调用」：

1. 预习 Spring AI **Tool Calling** 调用流程与参数 Schema（`@Tool` 注解、`ToolCallback` 机制）。
2. **优先偿还技术债 #1**（conversationId 归属校验）——正好呼应第六周”模型决策 vs 应用授权”主题。
3. 预习安全面：**工具结果污染、间接提示注入**——和本周场景 6 的注入守护一脉相承。
4. 定选题：只读工具三选一（天气 / 订单 / 知识库统计）。
5. 装配预研：确认给 ChatClient 挂 tools 的方式——大概率新增带 tools 的 client bean，**复用本周”方式 B 多 bean”经验**。
