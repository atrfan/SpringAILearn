...
> **学习日期：** 2026-06-20～2026-06-24
>
> **学习阶段：** 第 1 周 · 第 1～7 天
>
> **文档定位：** 第一周学习笔记与最终验收报告。记录理论结论、代码实现、实验数据、故障测试、技术债务和第二周准备事项。
>
> **本节目标：** 理解 LLM 的基本概念，使用 Java 21 原生 `HttpClient` 实现可复用、可观测、可测试的 DeepSeek API 客户端，通过受控实验分析 Temperature、Token、延迟与成本，并使用 WireMock 验证故障分类和有限重试。
...
- [8. Day05 任务覆盖与成本分析](#8-day05-任务覆盖与成本分析)
- [9. Day06 故障模拟与有限重试](#9-day06-故障模拟与有限重试)
- [10. 第一周验收与后续计划](#10-第一周验收与后续计划)
- [11. 关键结论](#11-关键结论)
- [12. 自测问题](#12-自测问题)
- [13. 参考资料](#13-参考资料)
...
Day06 在原始 `DeepSeekClient` 外增加 `RetryingChatClient` 装饰器。HTTP 协议、JSON 解析和错误分类仍由原始客户端负责；重试包装器只根据稳定的 `LlmErrorType` 决定是否重试。这样可以避免把退避、休眠和重试次数混入协议代码。

主要实现包括：

- `LlmChatClient`：统一原始客户端与重试客户端的调用接口；
- `RetryPolicy`：最大尝试次数、基础延迟、最大延迟、指数退避和抖动；
- `RetryingChatClient`：有限重试、`Retry-After`、中断处理和端到端耗时；
- `RetrySleeper`：可替换等待器，使测试不需要真实休眠；
- `RetryDecision`：错误处理决策表；
- `LlmClientException.attempts`：记录实际尝试次数；
- `CallResult.attempts`：记录成功恢复前的总尝试次数。

### 9.2 错误处理决策

| 故障 | 错误类型 | 自动重试 | 处理原则 |
|---|---|---:|---|
| HTTP 400、404 等 | `CLIENT_ERROR` | 否 | 修正参数、URI 或模型名称 |
| HTTP 401、403 | `AUTHENTICATION` | 否 | 修正密钥或权限 |
| HTTP 408、客户端请求超时 | `REQUEST_TIMEOUT` | 是 | 在总时间预算内有限重试 |
| HTTP 429 | `RATE_LIMIT` | 是 | 优先遵守 `Retry-After` |
| HTTP 5xx | `SERVER_ERROR` | 是 | 指数退避、抖动、限制次数 |
| 暂时性网络异常 | `NETWORK_ERROR` | 是 | 评估重复提交风险后有限重试 |
| 非法 JSON、关键字段缺失 | `RESPONSE_FORMAT` | 否 | 保存脱敏样本并修正协议映射 |

默认策略最多尝试 3 次，基础等待 1 秒，单次等待上限 10 秒。`Retry-After` 同时支持整数秒和 RFC 1123 HTTP 日期，并受本地最大等待上限约束。

### 9.3 响应结构校验

Day03 的 `ChatResponse.answer()` 会把缺少 `choices`、`message` 或 `content` 的响应转换为空字符串。这虽然避免了空指针异常，却会把协议损坏伪装成模型正常返回空答案。

Day06 在反序列化后增加关键字段校验：

- `model` 必须存在；
- `choices` 必须非空；
- `choices[0].message.content` 必须存在且非空；
- `usage` 及三类 Token 字段必须存在；
- Token 数不能为负数；
- `total_tokens` 必须等于输入与输出 Token 之和。

不满足这些条件时统一抛出不可重试的 `RESPONSE_FORMAT`，不会进入正常业务或盲目重试。

### 9.4 自动化测试结果

2026-06-24 执行 `mvn test`：
...
其中 `DeepSeekClientWireMockTest` 包含 17 个本地故障测试，覆盖：

- 401、400、408、429、500、503；
- 客户端请求超时；
- 非法 JSON；
- 缺少 `usage`；
- 空 `choices`；
- Token 用量不一致；
- 503 或 429 后恢复成功；
- `Retry-After` 秒数和 HTTP 日期；
- `Retry-After` 最大等待上限；
- 重试等待被中断。

所有测试均使用本地 WireMock，不访问真实 API，不消耗 Token。

### 9.5 重试的工程边界

自动重试不是无条件提高可靠性。Chat Completions 使用 POST，请求可能已经在服务端执行、计费或生成结果，只是客户端没有成功接收响应。网络异常后重试可能造成重复计算和重复计费。

当前客户端适合无外部副作用的文本生成调用。未来工具调用、订单提交或数据库写入场景必须增加幂等键、确认机制或业务去重，不能直接复用相同重试策略。
...
| 输出 Token 总计 | 7,074 |
| Token 总计 | 9,669 |
| Day05 可追溯估算成本 | $0.007991 |
| 自动化测试 | 34 项全部通过 |
| WireMock 故障测试 | 17 项 |

Day04 没有保存当日价格快照，因此不能把 Day04 和 Day05 拼成一个可追溯的总成本。只报告 Day05 成本比凭空补算更严谨。

### 10.2 第一周完成定义

- [x] 原生 Java API 调用成功；
- [x] API Key 通过环境变量读取，未发现真实密钥进入仓库；
- [x] 记录模型、HTTP 状态、输入/输出/总 Token、耗时和尝试次数；
- [x] 完成至少 20 条输入，实际完成 21 条；
- [x] 完成至少 15 次参数实验，实际完成 30 次；
- [x] 覆盖 401、429、5xx、超时、非法 JSON 和关键字段缺失；
- [x] 对可重试错误执行有限重试，对认证和格式错误禁止重试；
- [x] 形成 Temperature、Token、延迟、成本、幻觉和失败样本总结；
- [x] `mvn test` 可重复运行，34 项测试全部通过；
- [x] 本笔记已整合实验结论、故障测试、技术债务和第二周准备清单，作为第一周最终报告。

### 10.3 技术债务

| 优先级 | 技术债务 | 风险 |
|---|---|---|
| P0 | 未设置端到端重试总时间预算 | 多次超时叠加后阻塞时间过长 |
| P0 | 工具调用或写操作没有幂等机制 | 重试可能造成重复副作用 |
| P1 | 未记录脱敏结构化日志和指标 | 生产故障难以定位 |
| P1 | 旧实验未记录 `finish_reason` 和推理 Token | 无法严格解释空回答 |
| P1 | 自动评价器依赖简单规则 | 可能产生假阴性或错误结论 |
| P2 | 没有异步和流式响应 | 无法评估首 Token 延迟与客户端取消 |
| P2 | 没有 Maven Wrapper | 不同开发机 Maven 版本可能漂移 |
| P2 | 原生客户端与实验代码仍有配置重复 | 第二周应对照 Spring AI 抽象进行收敛 |

### 10.4 第二周准备清单

- [ ] 保留原生客户端作为比较基准，不在接入 Spring AI 时删除；
- [ ] 创建独立 Spring Boot/Spring AI 模块；
- [ ] 固定 Java、Spring Boot、Spring AI 和模型配置版本；
- [ ] 实现同步 `/api/chat`；
- [ ] 实现流式 `/api/chat/stream`；
- [ ] 普通测试使用 Mock，少量受控集成测试调用真实 API；
- [ ] 比较原生客户端与 Spring AI 在配置、异常、Token 观测和供应商耦合方面的差异；
- [ ] 测试客户端取消、上游超时和流式异常；
- [ ] 不把框架抽象误认为不同模型供应商的能力完全一致。

### 10.5 第一周最终判断

第一周最重要的结果不是写出一个聊天程序，而是建立了可复现的实验与故障验证方法。当前代码和数据能够支持以下判断：

- 低 Temperature 不等于事实正确；
- HTTP 200 不等于业务成功；
- `completion_tokens` 不等于可见文本长度；
- 自动评价结果不等于人工语义判断；
- 重试必须由错误类型、次数、退避、时间预算和幂等约束共同决定。

这些结论构成第二周引入 Spring AI 时的比较基线。
...
29. 可重试属性只表示故障可能具有暂时性，重试仍必须受最大次数、退避、抖动、`Retry-After` 和总时间预算约束。
30. HTTP 200 但缺少关键字段属于协议失败，不应转换为空回答继续流入业务。
31. 重试后的成功耗时应包含失败尝试和等待时间，否则性能统计会系统性低估故障恢复成本。
32. POST 重试可能造成重复计算和重复计费；带外部副作用的工具调用还需要业务幂等。
...
- [ ] 我能否解释为什么401和响应格式错误不应重试？
- [ ] 我能否说明指数退避、随机抖动和`Retry-After`分别解决什么问题？
- [ ] 我能否解释为什么HTTP 200但缺少`usage`仍然是失败？
- [ ] 我能否说明POST请求重试可能导致哪些重复执行风险？
- [ ] 我能否解释成功恢复后的耗时为什么必须包含前序失败和等待？
