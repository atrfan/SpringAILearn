...
>
> **当前进度：** Day09 已完成，同步 `/api/chat` 端点可用。
...
这种设计避免把供应商异常细节泄漏给客户端，但也损失了认证失败、模型不存在、配额耗尽等具体类别。Day10 测试阶段需要进一步观察 Spring AI 实际抛出的异常类型，再决定是否细分错误码。
