...
> **2026-06-29 收尾调整：** 第二周学习内容到 Day12 的流式响应与 Token 观测为止。Day12 中剩余的自动化边界测试、Day13 原生客户端与 Spring AI 的完整对比分析、Day14 全量验收与第三周衔接不再在本周继续执行。本周最终产出以同步接口、流式接口、流式 Token 观测笔记和已完成的排查记录为准。
...
> **收尾状态：** 本日实际只保留流式响应实现、流式 `ChatResponse` Token 观测和学习笔记整理。自动化测试扩展、取消/超时/异常的进一步测试不再继续。

**执行任务：**

1. ~~使用 `WebTestClient` 或类似工具测试流式端点。~~（本周不再继续）
2. ~~验证正常流式输出、中途取消、超时和异常场景。~~（本周不再继续）
3. ~~对比第一周记录的连接超时与请求超时行为。~~（本周不再继续）
4. 记录 Spring AI 在流式场景下暴露的 Token 观测能力（是否可获取输入/输出 Token）。

**产出：** Token 观测能力记录、流式响应排查记录、前端手动演示页面。流式测试用例和边界场景测试结果不再作为本周产出要求。

**验收：**

- [ ] ~~流式测试覆盖正常、取消、超时和异常场景~~（本周不再继续）
- [x] 记录流式响应是否支持 Token 统计
- [ ] ~~对比原生客户端与 Spring AI 在超时处理上的差异~~（本周不再继续）
- [ ] ~~所有测试通过~~（本周不再作为收尾要求）
...
> **收尾状态：** 本日内容不再执行。完整对比分析暂不作为第二周交付物。
...
- [ ] ~~对比表覆盖至少 8 个维度~~（本周不再继续）
- [ ] ~~每个维度有具体代码或行为示例~~（本周不再继续）
- [ ] ~~不把"框架更简洁"等同于"框架更好"，需说明代价~~（本周不再继续）
- [ ] ~~记录至少 3 个 Spring AI 隐藏的重要细节~~（本周不再继续）

## 第 14 天：7 月 1 日，周三（1 小时）

**目标：** 完成验收、总结并衔接第三周。

> **收尾状态：** 本日内容不再执行。全量测试、完整验收和第三周准备不再作为本周任务。
...
- [ ] ~~一条 Maven 命令可以构建并测试~~（本周不再继续）
- [x] 同步和流式端点均可用
- [ ] ~~完成对比分析报告~~（本周不再继续）
- [ ] ~~能说明 Spring AI 隐藏了哪些协议细节~~（本周不再继续）
- [ ] ~~能回答"什么时候该用框架，什么时候该用原生"~~（本周不再继续）

## 第二周完成定义

- [x] Spring Boot + Spring AI 项目可正常启动
- [x] API Key 未进入仓库
- [x] 同步 `/api/chat` 可用，响应包含 Token 和耗时
- [x] 流式 `/api/chat/stream` 可用，支持逐块返回
- [ ] ~~Mock 测试覆盖正常、异常和边界场景~~（本周不再继续）
- [ ] ~~集成测试通过 profile/tag 隔离，默认不调用真实 API~~（本周不再继续）
- [ ] ~~完成原生客户端与 Spring AI 的多维度对比~~（本周不再继续）
- [x] 记录 Spring AI 在异常、Token 观测和供应商耦合方面的限制
- [ ] ~~所有自动化测试可重复运行~~（本周不再继续）
- [ ] ~~形成技术债务和第三周衔接清单~~（本周不再继续）
